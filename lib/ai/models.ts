...
//   against `max_tokens`.
// - Reasoning defaults to off for every task, a deliberate deviation from CLAUDE.md section 7 (which asks for
//   "low" on analysis tasks). Measured on the scholarship reconcile prompt (1,430 input tokens, September 2026):
//   reasoning off returned in 2 s; reasoning low spent 16,000 tokens (235 s) on the reasoning trace without
//   producing an answer, which cannot fit the 300 s function limit on Vercel. Rule-based findings are
//   computed deterministically instead (supersessions, missing annexures, coverage, RTS). Reasoning can still be
//   switched on per task in Settings for local use; the token budget is raised when it is.
...
export const TASKS: Record<AiTask, TaskConfig> = {
  ...
  reconcile: { label: 'Source reconciliation', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0.1, timeoutMs: 120_000 },
  discovery: { label: 'Discovery and gap checklist', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0.2, timeoutMs: 120_000 },
  questions: { label: 'Clarifying questions', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0.3, timeoutMs: 120_000 },
  workflow: { label: 'Workflow proposal', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0.1, timeoutMs: 120_000 },
  fields: { label: 'Data dictionary from form', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0, timeoutMs: 90_000 },
  rules: { label: 'Decision tables', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0, timeoutMs: 120_000 },
  ...
  requirements: { label: 'Requirements per module', model: HEAVY, reasoning: 'off', maxTokens: 12000, temperature: 0.2, timeoutMs: 200_000 },
  review: { label: 'Semantic quality review', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0.1, timeoutMs: 120_000 },
  fix: { label: 'Fix and short rewrites', model: FAST, reasoning: 'off', maxTokens: 2000, temperature: 0.2, timeoutMs: 45_000 },
  impact: { label: 'Corrigendum impact analysis', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0.1, timeoutMs: 120_000 },
  cr: { label: 'Vendor CR scope check', model: HEAVY, reasoning: 'off', maxTokens: 8000, temperature: 0.1, timeoutMs: 120_000 },
...
/** Work budget inside a route, leaving time to send the response. */
export const ROUTE_BUDGET_MS = 280_000;
...
export function taskConfig(task: string, reasoningOverride?: Reasoning): TaskConfig {
  ...
  const maxTokens = reasoningOverride === 'off' ? base.maxTokens : Math.max(base.maxTokens, 24000);
  return { ...base, reasoning: reasoningOverride, maxTokens, timeoutMs: Math.max(base.timeoutMs, 240_000) };
}
